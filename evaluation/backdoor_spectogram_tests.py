...
from data.spectograms import SpectogramDataset, LoadImages, Spectogram_Loader
...
def run(dataset, model):
    ...
    run_defense(
        dataset,
        model_ctor, 
        [filter_op_ctr, train_op_ctr], Spectogram_Loader, 
        10, 4, 
        device, [filter_s_ctr, train_s_ctr])

if __name__ == "__main__":
    exp_id = int(sys.argv[1]) # unused for now
    model = sys.argv[2]

    image_paths, labels = LoadImages()
    # Crear el dataset con las rutas de imágenes, etiquetas y transformaciones
    dataset = SpectogramDataset(image_paths=image_paths, labels=labels)
